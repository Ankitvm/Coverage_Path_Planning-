...
path_covered(wght-1) = max(path_iter);
pass_time(wght-1) = max(pass_time_cum);
energy(wght-1) = max(energy_cum);
...
subplot(3,1,1)
box on;
path1_covered = path_covered*20;
plot(0.1:0.05:0.9,[ path1_covered])
title('Area Coverage v/s. Weight')
ylim([95 105]);
xlabel('Weight, w')
ylabel('Length')
grid on
...
subplot(3,1,2)
box on;
plot(0.1:0.05:0.9, [pass_time]);
title('Total Coverage Time v/s. Weight')
xlabel('Weight, w')
ylabel('Time Taken')
grid on
...
subplot(3,1,3)
box on;
plot(0.1:0.05:0.9,[energy])
title('Total Control Energy v/s. Weight')
xlabel('Weight, w')
ylabel('Energy')
grid on
% xlim([0 iter+2])