...
path_covered(obsize) = max(path_iter);
pass_time(obsize) = max(pass_time_cum);
energy(obsize) = max(energy_cum);
...
subplot(3,1,1)
box on;
plot(1:10,[1/path_covered])
title('Coverage Path Length Ratio v/s. Obstacle Size')
% ylim([ 105]);
xlabel('Obstacle Size, r_{obs}')
ylabel('Coverage Path Length Ratio, (Path_cov_obs/Path_no_obs)')
grid on
...
box on;
plot(1:10, [pass_time]);
title('Total Coverage Time v/s. Obstacle Size')
xlabel('Obstacle Size, r_{obs}')
ylabel('Time Taken')
grid on
...
box on;
plot(1:10,[energy])
title('Total Control Energy v/s. Obstacle Size')
xlabel('Obstacle Size, r_{obs}')
ylabel('Energy')
grid on
% xlim([0 iter+2])