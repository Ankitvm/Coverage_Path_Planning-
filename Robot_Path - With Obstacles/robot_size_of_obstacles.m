...
%Obstacle Parameters ------------------------------------------------------ 
rad = (obsrad^2)*ones(10,1);% 0.04*rand(10,1)+0.005;
xrnd = 8*rand(10,1)+1;
yrnd = 8*rand(10,1)+1;
...
area = integrate(sqrt(dot(x1)^2+dot(x2)^2));
% Objective----------------------------------------------------------------
objective = tot_cost;
...
area_covered(round(obsrad/0.1)) = sum(area_iter);
pass_time(round(obsrad/0.1)) = sum(time_iter);
energy(round(obsrad/0.1)) = sum(energy_iter);
end
...
subplot(3,1,1)
box on;
plot(0:0.1:2,[0 area_covered])
title('Iteration-wise Area coverage')
xlabel('Size of Obstacles')
ylabel('Area')
grid on
